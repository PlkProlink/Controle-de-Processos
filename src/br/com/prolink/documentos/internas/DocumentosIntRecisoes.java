 ...
 * @author Tiago Dias
 */
public class DocumentosIntRecisoes extends javax.swing.JInternalFrame {
            ...
            JOptionPane.showMessageDialog(null, "3º data de envio não é permitido se a 1º e 2º não forem informadas");
                ...
                if(data1.trim().length()==10 
                    ...
                    try{
                        ...
                        String gry = "insert into recisao "
    ...
    private void btnExcluirActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnExcluirActionPerformed
        if(txtCodigo.getText().equals("")){
            ...
            if(escolha==JOptionPane.YES_OPTION){
                try{
                    int excluiu = conexao.statement.executeUpdate("delete from recisao where Cod="+txtCodigo.getText());
                    if(excluiu == 1){
    ...
    try {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new DocumentosIntRecisoes().setVisible(true);
            }
        });
    }
...
public void pegar_ultimo_registro(){
        ...
        conexao.executeSQL("select * from recisao where NumeroProcesso='"+processo+"'");
        try {
...
public void atualizar_documentos1(){
    ...
    conexao.statement.executeUpdate("update documentos set "
                                        + "Recisao='Enviado para o Cliente' where Numerodoprocesso='"+processo+"'");
    }catch(SQLException erro){
...
public void atualizar_documentos2(){
    try{
    conexao.statement.executeUpdate("update documentos set "
                                        + "Recisao='Aguardando Validação' where Numerodoprocesso='"+processo+"'");
    }catch(SQLException erro){
        JOptionPane.showMessageDialog(null, "Erro ao atualizar documentos: tela>" +logb.getTela());
    }
}
public void atualizar_documentos3(){
    try{
    conexao.statement.executeUpdate("update documentos set "
                                        + "Recisao='Validação Efetuada' where Numerodoprocesso='"+processo+"'");
    }catch(SQLException erro){
        JOptionPane.showMessageDialog(null, "Erro ao atualizar documentos: tela>" +logb.getTela());
    }
}
