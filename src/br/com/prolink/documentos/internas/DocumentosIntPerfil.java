 ...
 * @author Tiago Dias
 */
public class DocumentosIntPerfil extends javax.swing.JInternalFrame {
    ...
    private void initComponents() {
        ...
        setClosable(true);
        setTitle("Senhas/Autorizações - Configuração de Perfil Fiscal no Portal Prefeitura");
        try {
            setSelected(true);
    ...
    private void btnExcluirActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnExcluirActionPerformed
        if(txtCodigo.getText().equals("")){
            ...
            if(escolha==JOptionPane.YES_OPTION){
                try{
                    int excluiu = conexao.statement.executeUpdate("delete from configurarperfilfiscalnositeprefeituradocumento where Cod="+txtCodigo.getText());
                    if(excluiu == 1){
    ...
    try {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new DocumentosIntPerfil().setVisible(true);
            }
        });
    }
...
public void pegar_ultimo_registro(){
        ...
        conexao.executeSQL("select * from configurarperfilfiscalnositeprefeituradocumento where NumeroProcesso='"+processo+"'");
        try {
...
public void atualizar_documentos1(){
    ...
    conexao.statement.executeUpdate("update documentos set "
                                        + "ConfigurarPerfilFiscalNoSitePrefeituraDocumento='Enviado para o Cliente' where Numerodoprocesso='"+processo+"'");
    }catch(SQLException erro){
...
public void atualizar_documentos2(){
    try{
    conexao.statement.executeUpdate("update documentos set "
                                        + "ConfigurarPerfilFiscalNoSitePrefeituraDocumento='Aguardando Validação' where Numerodoprocesso='"+processo+"'");
    }catch(SQLException erro){
        JOptionPane.showMessageDialog(null, "Erro ao atualizar documentos: tela>" +logb.getTela());
    }
}
public void atualizar_documentos3(){
    try{
    conexao.statement.executeUpdate("update documentos set "
                                        + "ConfigurarPerfilFiscalNoSitePrefeituraDocumento='Validação Efetuada' where Numerodoprocesso='"+processo+"'");
    }catch(SQLException erro){
        JOptionPane.showMessageDialog(null, "Erro ao atualizar documentos: tela>" +logb.getTela());
    }
}
