...
import br.com.prolink.controle.SessaoUsuarioBean;
...
public class CadastroClassificacao extends javax.swing.JFrame {
    ...
    LogUsuarioBean logU = new LogUsuarioBean();
    SessaoUsuarioBean su = new SessaoUsuarioBean();
    ...
    String usuario=su.getUsuario(), tela="Cadastro de Classificação", descricao="Novo registro", acao="Inserir", enviaDataBanco;
    ...
    public void pega_tempo_e_grava(){
        ...
        enviaDataBanco = ano+"-"+mes+"-"+dia;
        
        String hora = sh.format(dt);
        JOptionPane.showMessageDialog(null, "Data:" +enviaDataBanco+ "\nHora: "+hora);
        
        try{
            con_classificacao.exeQuery("insert into log_usuario (Usuario,Tela,Acao,Descricao,Data,Hora, Endereco, Hostname) values ('"+
            usuario+"','"+tela+"','"+acao+"','"+descricao+"','"+enviaDataBanco+"','"+hora+"')");
        }catch(Exception add){
            JOptionPane.showMessageDialog(null,"Erro "+add);
        }
}        
